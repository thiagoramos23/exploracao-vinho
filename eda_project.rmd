---
Nome: Thiago de Lima Viana Ramos
...
```{r echo=FALSE, plot-fixed-acidity}
...
```{r echo=FALSE, plot-volatile-acidity}
...
```{r echo=FALSE, plot-residual-sugar}
...
```{r echo=FALSE, box-plor-total-sulfur}
boxplot(redwine$total.sulfur.dioxide, horizontal = T, frame.plot = FALSE,
        main='Total de Dióxide de Enxofre', pch=21, cex = .4)
```
...
The dataset has 1599 observations and 13 variables.
...
Será que o PH influencia na qualidade dos vinnhos?
...
O pH descreve o quão ácido o vinho é. Aqui podemos ver que não existe uma boa correlação entre essas variáveis já que existem vinhos com qualidade 6 com pH alto e com pH baixo. Essa variável está logo descartada.
...
É muito interessante notar aqui que a partir da qualidade 5, ou seja, partindo dos vinhos medianos em diantes a qualidade está bem relacionada a quantidade de algo. A correlação não é forte já que a grande maioria dos vinhos fica na média de 5 ou 6 mas podemos ver que quanto maior a qualidade do vinho, maior é a quantidade de álcool.
...
Interessante notar que quanto menor a quantidade de ácidez, melhor o vinho. De acordo com uma pesquisa feitas em sites especializados em vinho, a volatilidade da acidez é o que faz um bom vinho. A quantidade correta de acidez deixa o vinho mais fresco, dá forma e sabor, enfim, é a alma do vinho. Um vinho bom tem a quantidade certa de acidez e isso pode ser visto através do gráfico acima. Apararentemente uma média de 0.4g/dm^3 é o ideal, e os melhores vinhos parecem estar entre 0.3 e 0.5g/dm^3
...
Aqui eu quis entender se a densidade de água continha alguma correlação com o álcool. Vemos pelo gráfico que quanto mais água, menos a porcentagem de álcool.
...
### Descrição
Quanto mais densidade de água, menor a quantidade de álcool o que corresponde á um vinho de menor qualidade. Temos um ou outro outlier, o que prova que toda regra tem sua exceção, mas em geral a densidade X álcool afeta a qualidade do vinho.
